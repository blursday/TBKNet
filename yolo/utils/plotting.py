...
from tbknet.yolo.utils import LOGGER, TryExcept, plt_settings, threaded
...
class Colors:
    ...
    def __init__(self):
        ...
        # hexs = ('FF3838', '00C2FF', 'FF9D97', 'FF701F', '392ed1', 'CB38FF', 'e10000',  '3DDB86','FFB21D', 'FF95C8', 'FF37C7',
        #         '00D4BB','3d941b','3DDB86','1A9334','d15926','FFB21D', 'CFD231', '48F90A', '92CC17',)
...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font='Arial.ttf', pil=False, example='abc'):
        ...
        if self.pil:  # use PIL
            ...
            self.im = im
        #self.lw = line_width or max(round(sum(im.shape) / 20 * 0.003), 2)  # line width
        self.lw = line_width or max(round(sum(im.shape) / 20), 2)  # line width
        # Pose
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                tf = max(self.lw - 1, 1)*2  # font thickness
                #w, h = cv2.getTextSize(label, 0, fontScale=self.lw / 4, thickness=tf)[0]  # text width, height
                w, h = cv2.getTextSize(label, 0, fontScale=self.lw/2.5 , thickness=tf)[0]
                outside = p1[1] - h >= 3
    ...
    def kpts(self, kpts, shape=(960, 960), radius=10, kpt_line=True):
...
def plot_images(images,
    ...
    #fs = int((h + w) * ns )  # font size
    annotator = Annotator(mosaic, line_width=round(fs / 10), font_size=fs, pil=True, example=names)
    for i in range(i + 1):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
        annotator.rectangle([x, y, x + w, y + h], None, (255, 255, 255), width=4)  # borders
        if paths:
