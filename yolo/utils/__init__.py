...
from tbknet import __version__
...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(f"""
            '{name}' object has no attribute '{attr}'. This may be caused by a modified or out of date tbknet
            'default.yaml' file.\nPlease update your code with 'pip install -U tbknet' and if necessary replace
...
def get_settings(file=SETTINGS_YAML, version='0.0.3'):
    ...
    from tbknet.yolo.utils.checks import check_version
    from tbknet.yolo.utils.torch_utils import torch_distributed_zero_first
    ...
    with torch_distributed_zero_first(RANK):
        ...
        if not correct:
            LOGGER.warning('WARNING ⚠️ Ultralytics settings reset to defaults. This is normal and may be due to a '
                           'recent tbknet package update, but may have overwritten previous settings. '
...
def set_settings(kwargs, file=SETTINGS_YAML):
    """
    Function that runs on a first-time tbknet package installation to set up global settings and create necessary
    directories.
    """
...
def deprecation_warn(arg, new_arg, version=None):
    ...
    LOGGER.warning(f"WARNING ⚠️ '{arg}' is deprecated and will be removed in 'tbknet {version}' in the future. "
