...
from tbknet.nn.fb import FocalPWConvBlock, FocalNeXtBlock, FocalPWConvBlock1, FocalPWConvBlock2
from tbknet.nn.mobileone import MobileOneBlock
from tbknet.yolo.utils.tal import dist2bbox, make_anchors
from tbknet.nn.GhostV2 import Ghostblockv2
...
class Ghost2f(nn.Module):
    # Ghost2f Bottleneck with 2 convolutions
...
class CBAM(nn.Module):
    ...
    def __init__(self, c1, kernel_size1=7):  # ch_in, kernels
        super().__init__()
        self.channel_attention = ChannelAttention(c1)
        self.spatial_attention = SpatialAttention(kernel_size1=7)
...
class FocalPWConv(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(FocalPWConvBlock(self.c) for _ in range(n)))
...
class FocalPWConvC2(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv(2 * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(FocalPWConvBlock1(self.c) for _ in range(n)))
...
class FocalPWConvC21(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv(2 * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(FocalPWConvBlock2(self.c) for _ in range(n)))
