...
def get_activation(name='silu', inplace=True):
    ...
    if isinstance(name, str):
        if name == 'silu':
            ...
            module = H_swish(inplace=inplace)
...
class RepModule(nn.Module):
    def __init__(self,
        ...
        self.conv3 = ConvBNAct(ch_in, ch_hidden, 1, act=act)
...
class C3Rep2N(nn.Module):
    ...
    def __init__(self,
        ...
        next_ch_in = ch_mid
        for i in range(n):
            if block_fn == 'RepModule':
                self.convs.add_module(
                    str(i),
                    RepModule(next_ch_in,
                                           ch_hidden_ratio,
                                           ch_mid,
                                           act=act,
                                           shortcut=True))
